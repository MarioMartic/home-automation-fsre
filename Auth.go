...
func SignIn(c *gin.Context) {
	...
	if err := db.Debug().Raw("select u.*, count(um.user_id) as microcontrollers_count from users u join users_microcontrollers um on um.user_id = u.id where u.email=? GROUP BY u.* ", creds.Email).Scan(&user).Error; err != nil {
